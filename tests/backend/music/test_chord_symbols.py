...
class TestKind:
    def test_minor_appends_m(self, profile, ctx):
        cells = translate_tree(_harmony("D", kind="minor"), ctx, profile)
        # 'd' + 'm'
        assert _dots(cells) == [(1, 4, 5), (1, 3, 4)]

    def test_dominant_appends_7(self, profile, ctx):
        # G7 → 'g' + lower-row '7'
        cells = translate_tree(_harmony("G", kind="dominant"), ctx, profile)
    ...
    def test_half_diminished_uses_half_diminished_entity(
        ...
        cells = translate_tree(_harmony("C", kind="half-diminished"), ctx, profile)
        # 'c' + circle + dot
    ...
    def test_unknown_kind_emits_bare_root_with_warning(self, profile, ctx):
        ...
        cells = translate_tree(_harmony("C", kind="exotic-chord"), ctx, profile)
...
class TestBass:
    def test_c_over_g(self, profile, ctx):
        # C/G → 'c' + slash + 'g'
        cells = translate_tree(_harmony("C", bass_step="G"), ctx, profile)
        assert _dots(cells) == [
            (1, 4),         # c
            (3, 4),         # slash
            (1, 2, 4, 5),   # g
...
class TestFeatureGate:
    def test_show_chord_symbols_false(self, profile, ctx, monkeypatch):
        ...
        cells = translate_tree(_harmony("C", kind="major"), ctx, profile)
        assert cells == []
...
class TestEdgeCases:
    def test_harmony_without_root_warns(self, profile, ctx):
        ...
        cells = translate_tree(h, ctx, profile)
        assert cells == []
        codes = [w.code for w in ctx.warnings.warnings]
        assert "MUSIC_UNSUPPORTED_NOTATION" in codes
