...
def test_chord_root_resets_across_measures(profile, ctx):
    ...
    translate_tree(part, ctx, profile)
...
class TestChordDirectionByClef:
    ...
    def test_treble_writes_top_note(self, profile, ctx):
        cells = translate_tree(self._chord_with_clef("G", 2), ctx, profile)
    ...
    def test_clef_changes_written_note_not_interval_sizes(self, profile, ctx):
        ...
        treble = translate_tree(self._chord_with_clef("G", 2), ctx, profile)
        bass = translate_tree(self._chord_with_clef("F", 4), ctx, profile)
