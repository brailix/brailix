...
def test_blank_cell_between_consecutive_measures(profile, ctx):
    cells = translate_tree(_score(_NOTE, _NOTE, _NOTE), ctx, profile)
    seps = _seps(cells)
...
def test_no_separator_before_first_measure(profile, ctx):
    cells = translate_tree(_score(_NOTE), ctx, profile)
    assert _seps(cells) == []
...
def test_separator_none_disables(profile, ctx, monkeypatch):
    ...
    cells = translate_tree(_score(_NOTE, _NOTE), ctx, profile)
    assert _seps(cells) == []
...
def test_separator_coexists_with_explicit_barline(profile, ctx):
    ...
    cells = translate_tree(_score(m1, _NOTE), ctx, profile)
